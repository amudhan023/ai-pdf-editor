...
A standalone SwiftPM executable exists (P1-08): `main.swift` proves `Platform`'s XPC transport in a real, separately-launchable process via a startup self-check. Not yet a real `.xpc` bundle and does not accept cross-process connections; the store/key-hierarchy/lock logic it will host lives in `Packages/VaultStore`.
