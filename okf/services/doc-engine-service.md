...
Routing this service's requests through `DocEngineHost`'s real `PDFiumEngine` (the engine itself now does lifecycle/tiled-render/outline — see [../engines/doc-engine-host.md](../engines/doc-engine-host.md) — but is wired in-process by `App/`, not through this service). The real `.xpc` bundle registration into the app target.
