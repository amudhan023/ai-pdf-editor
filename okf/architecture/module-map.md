...
| `InferenceAPI` | Typed inference request/response protocols | Contain a real model implementation | **Scaffolded (stub)** — see [packages/inference-api.md](../packages/inference-api.md) |
...
| `Platform` | Keychain, LAContext, file coordination, XPC transport, domain event bus | — | **Partial** — XPC transport implemented, rest not yet — see [packages/platform.md](../packages/platform.md) |
| `DocumentSession` | Document lifecycle, undo/redo, dirty state, recovery journal | Perform PDF byte manipulation itself | **Scaffolded (stub)** — see [sessions/document-session.md](../sessions/document-session.md) |
...
| `AutofillEngine` | Field discovery, semantic matching, value formatting, fill plan construction | Write into the document | **Scaffolded (stub)** — see [engines/autofill-engine.md](../engines/autofill-engine.md) |
...
| `AuditLog` | Append-only, hash-chained local log | Log field values | **Scaffolded (stub)** — see [packages/audit-log.md](../packages/audit-log.md) |
| `DocEngineHost` | XPC client + PDFium adapter implementing `PDFEngineAPI` | Touch network/vault/unhanded files | **Scaffolded (stub)** — see [engines/doc-engine-host.md](../engines/doc-engine-host.md) |
| `InferenceHost` | Model registry, typed inference endpoints implementing `InferenceAPI` | Load unchecksummed models; make network calls | **Scaffolded (stub)** — see [engines/inference-host.md](../engines/inference-host.md) |
| `VaultStore` | SQLCipher DB, key hierarchy, lock state, crypto-shred | Return bulk plaintext dumps | **Scaffolded (stub)** — see [engines/vault-store.md](../engines/vault-store.md) |
...
| `PrivacyDashboard` | Trust surface: stored-data summary, audit timeline, network disclosure | — | **Scaffolded (stub)** — see [ui/privacy-dashboard.md](../ui/privacy-dashboard.md) |
| `App` | App target: DI composition root, windows/menus | Contain business logic | **Not scaffolded** — no Swift files exist yet |
