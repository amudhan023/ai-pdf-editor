...
|---|---|---|---|---|
| `Vaultform.app` (main) | Hosts UI + coordinators + domain services + the Policy & Trust layer | App Sandbox, network only for two toggleable app-level paths (update check, license validation) | N/A | Minimal shell viewer app (P0-07): `AppDelegate`/`RootView` wiring `PDFiumEngine` in-process behind `PDFEngineAPI` |
| `DocEngine.xpc` | **Hostile input** — parses arbitrary PDFs/DOCX/images | No network, no vault container; receives security-scoped file handles only; one instance *per open document* | Lost render of one document; auto-restart | Skeleton self-check `main.swift` (see [services/doc-engine-service.md](../services/doc-engine-service.md)); the PDFium engine now exists in `DocEngineHost` but the app wires it in-process — `.xpc` bundle registration still not built |
| `Inference.xpc` | Semi-trusted; processes extracted content | No network; read-only model directory; memory-capped | In-flight inference retried | Skeleton self-check `main.swift` (P1-12); real registry/router logic lives in `InferenceHost` |
| `Vault.xpc` | **Most privileged** — sole owner of vault DB and keys | No network; exclusive vault container access; talks only to the main app's Policy layer | Vault relocks; DB is transactional | Skeleton self-check `main.swift` (P1-08); real store/key-hierarchy logic lives in `VaultStore` |
