...
|---|---|---|---|---|
| `Vaultform.app` (main) | Hosts UI + coordinators + domain services + the Policy & Trust layer | App Sandbox, network only for two toggleable app-level paths (update check, license validation) | N/A | `App/` has no Swift files yet — not scaffolded |
