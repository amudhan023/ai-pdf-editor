...
- `AnnotationStore` — CRUD over `Annotation` (subtype, bounding box, color, contents, author, modifiedAt, plus `quadPoints`/`opacity`/`createdAt` added additively by **ADR-014**, P1-04, for text-markup geometry, and `inkPaths`/`linkURL` added additively by **ADR-015**, P1-05, for freehand ink and link annotations). `AnnotationSubtype.link` also added by ADR-015. Implemented for real by `DocEngineHost` (see its okf page) as of P1-04/P1-05; `DocumentSession` wraps it as an optional capability with undo/redo.
