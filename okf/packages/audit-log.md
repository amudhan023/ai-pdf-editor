...
Implemented (P1-15, P1-18): `AuditEntry` (no value slot, by construction) and the append-only hash-chained `AuditLog`, plus a domain-event-bus adapter so privileged operations get logged from `Platform`'s `Events/` bus.
