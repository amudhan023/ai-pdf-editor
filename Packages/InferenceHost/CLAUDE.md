...
**Allowed imports:** Foundation, InferenceAPI, Platform, CryptoKit, NaturalLanguage (see `Scripts/import-allowlist.txt` — the enforced source of truth). Tests may also import XCTest.
...
- `embed` (P1-14) is backed by Apple's on-device `NLEmbedding`, not a vendored model pack — `manifest.packData` is deliberately unused in that path; `classify`/`extractEntities` are still stubs pending Phase 2.
