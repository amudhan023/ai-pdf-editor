...
**PDFium:** links `../../ThirdParty/pdfium/prebuilt/PDFium.xcframework` (a vendored prebuilt, not built from source — see `docs/adr/ADR-001-pdfium-source-and-pin.md`) via the `PDFium` binary target. `CPDFium` (`Sources/CPDFium`) is a thin header-only module-map target exposing `fpdfview.h` to Swift, since a raw-dylib xcframework's headers aren't auto-importable the way a `.framework`'s are — add more headers there only as real usage needs them, don't bulk-copy the whole PDFium header set speculatively.
