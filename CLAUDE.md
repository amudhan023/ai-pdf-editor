...
6. **API packages (`Packages/*API/`) and `Schemas/` are frozen seams.** Changing them requires an ADR and an `[INTEGRATION]`-marked PR — self-mergeable once the ADR is present and CI is green (ADR-008), no separate human-review click required. Never "just add a field" without the ADR.
...
- `main` is always releasable and **platform-protected**: GitHub branch protection requires the `ci-status` check (`verify.sh` matrix + repo-wide checks, see P0-02) green before the merge button unlocks (`strict: true` — the branch must also be up to date), configured per `tasks/escalations/E-003-branch-protection-needs-paid-plan.md`'s Resolution v2 (P0-14). `enforce_admins: false`, so the repo owner/agent can still self-merge without a separate required-reviewer count. Still check `ci-status` yourself before attempting a merge — GitHub will refuse it if you're wrong, but don't rely on trial-and-error. **Per ADR-008 (Constitution Articles 7/10 amendment): merge autonomously the moment `ci-status` is green — this now includes `[INTEGRATION]`, security-touching, and API/schema PRs, not just ordinary ones**, provided any ADR those articles require is present in the diff. **Per ADR-009: this now also includes changes to this file and `docs/AGENT_LOOP.md`** — no separate human-approval click required for those either. This is standing authorization, not a per-PR ask (see `docs/AGENT_LOOP.md` Step 8a). If `ci-status` is red, never merge: diagnose and fix per the normal fix loop (Step 4 there), then re-check before trying again. **Still always human-reviewed, no exception, and cannot be delegated away by any ADR:** new entitlements/Info.plist permission changes, and any change to `docs/CONSTITUTION.md` itself (its own Amendment clause is unconditional).
...
- This file (root CLAUDE.md) and `docs/AGENT_LOOP.md` are self-mergeable once `ci-status` is green, per ADR-009 — no separate human-approval click, though a dedicated PR touching nothing else is still the convention. `docs/CONSTITUTION.md` is the one doc this can never apply to: its own Amendment clause requires a human-merged PR, unconditionally.
