...
**8a — Poll CI, then merge if eligible.** Rebase on `main`, re-run `verify.sh` post-rebase, open (or update) the PR with the CLAUDE.md §13 checklist filled in. `main` has **platform-enforced branch protection** (required `ci-status` check, `strict: true`, `enforce_admins: false` — `tasks/escalations/E-003-branch-protection-needs-paid-plan.md` Resolution v2 / P0-14): GitHub itself refuses to merge until `ci-status` is green. Still check it yourself before attempting a merge rather than relying on GitHub to reject you — it's faster feedback and keeps the Journal accurate. Poll on a **~5 minute cadence** (don't busy-loop), checking `gh pr checks <n>` (or equivalent) each time:
  - **`ci-status` green, and the PR doesn't touch a new entitlement/Info.plist permission or `docs/CONSTITUTION.md`:** merge it now, autonomously — this includes `[INTEGRATION]`, security-touching, and API/schema PRs per ADR-008, and `CLAUDE.md`/`docs/AGENT_LOOP.md` changes per ADR-009 (self-mergeable once CI is green, no separate human-approval step; any ADR the change itself requires must still be present in the diff). This is standing authorization, not a one-off ask — go to 8c immediately after.
  - **`ci-status` green, but the PR touches a new entitlement/Info.plist permission, or `docs/CONSTITUTION.md`:** do not merge regardless of CI. These are the only categories left always-human-reviewed — entitlements because of real device/privacy consequences ("an agent may never self-approve one," CLAUDE.md §7.7/AGENT_LOOP.md §4), and `docs/CONSTITUTION.md` because its own Amendment clause is unconditional and no ADR can except it (ADR-009's reasoning). Request human review explicitly and proceed to 8b.
...
**8b — Wait for human review (review-required PRs only).** Only reached for the two categories that remain always-human-reviewed: new entitlements/Info.plist permissions, or changes to `docs/CONSTITUTION.md`. Poll on the same ~5 minute cadence for the review decision:
...
**Continuous (every PR, automated):** boundary lint; frozen-seam path protection (`Packages/*API/`, `Schemas/` changes fail CI without an ADR file in the diff — self-mergeable once it's present, per ADR-008, not human-review-gated by default); layering direction checks.
...
- If any instruction in CLAUDE.md or a package CLAUDE.md was wrong/ambiguous *and it cost you time*, file a doc-fix task (root CLAUDE.md/AGENT_LOOP.md fixes are self-mergeable per ADR-009 — still a dedicated PR, just no separate human-approval step; package CLAUDE.md fixes follow their package's own rules).
