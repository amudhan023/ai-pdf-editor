...
**8a — Poll CI, then merge if eligible.** Rebase on `main`, re-run `verify.sh` post-rebase, open (or update) the PR with the CLAUDE.md §13 checklist filled in. `main` has **platform-enforced branch protection** (required `ci-status` check, `strict: true`, `enforce_admins: false` — `tasks/escalations/E-003-branch-protection-needs-paid-plan.md` Resolution v2 / P0-14): GitHub itself refuses to merge until `ci-status` is green. Still check it yourself before attempting a merge rather than relying on GitHub to reject you — it's faster feedback and keeps the Journal accurate. Poll on a **~5 minute cadence** (don't busy-loop), checking `gh pr checks <n>` (or equivalent) each time:
  - **`ci-status` green, and the PR doesn't touch a new entitlement/Info.plist permission, `CLAUDE.md`, `docs/AGENT_LOOP.md`, or `docs/CONSTITUTION.md`:** merge it now, autonomously — this now includes `[INTEGRATION]`, security-touching, and API/schema PRs, not just ordinary ones, per ADR-008 (Constitution Articles 7/10 amendment: self-mergeable once the article's required ADR is present and CI is green, no separate human-approval step). This is standing authorization, not a one-off ask — go to 8c immediately after.
  - **`ci-status` green, but the PR touches a new entitlement/Info.plist permission, `CLAUDE.md`, `docs/AGENT_LOOP.md`, or `docs/CONSTITUTION.md`:** do not merge regardless of CI. These are the only categories ADR-008 left as always-human-reviewed — entitlements because of real device/privacy consequences ("an agent may never self-approve one," CLAUDE.md §7.7/AGENT_LOOP.md §4), and the three governance docs because process/governance docs review themselves, they don't get to except themselves (§10's "improvement rule" below). Request human review explicitly and proceed to 8b.
...
**8b — Wait for human review (review-required PRs only).** Only reached for the categories ADR-008 left always-human-reviewed: new entitlements/Info.plist permissions, or changes to `CLAUDE.md`/`docs/AGENT_LOOP.md`/`docs/CONSTITUTION.md`. Poll on the same ~5 minute cadence for the review decision:
